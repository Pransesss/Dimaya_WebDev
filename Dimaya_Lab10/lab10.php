<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['create_order'])) {
            ...
            $pizza_name = mysqli_real_escape_string($conn, $pizza['name']);
            ...
            if (!empty($_POST['toppings'])) {
                foreach ($_POST['toppings'] as $tid) {
                    $tid = (int)$tid;
                    $t = mysqli_fetch_assoc(mysqli_query($conn, "SELECT name, price FROM toppings WHERE id=$tid"));
            ...
            $toppings_str = mysqli_real_escape_string($conn, implode(', ', $topping_names));
            $total = ($pizza_price + $toppings_total) * $qty;
            
            // 4. INSERT the final order into the 'orders' table
            mysqli_query($conn, "INSERT INTO orders (customer, pizza, toppings, qty, total) VALUES ('$customer', '$pizza_name', '$toppings_str', $qty, $total)");
            header("Location: " . $_SERVER['PHP_SELF']); exit;
        }
                    ...
                    <tbody>
                        <?php
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                $details = "$pizza x$qty" . (!empty($toppings) ? "<br><small>+ $toppings</small>" : "");
